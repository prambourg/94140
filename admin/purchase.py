...
class FilterByProduct(BaseSQLAFilter):
    def apply(self, query, value, alias=None):
        return query.join(Purchase.product).filter(Product.id == value)

    def operation(self):
        return u'equals'

    def get_options(self, view):
        return [(product.id, f'{product.barcode} - {product.label}') for product in Product.query.all()]


def get_products_choices():
    products = Product.query.all()
    choices = ((product.id, f'{product.barcode} - {product.label}') for product in products)
    return choices
...
class PurchaseAdminView(AethModelView):
    ...
    column_editable_list = ('price', 'weight',)
    ...
    column_filters = [FilterByProduct(column=None, name='Produits'),]


    def format_product(self, context, model, name):
        return Markup('<a href="{url}" title="Link to Product">{product}</a>'.format(
            url=url_for('product.details_view', id=model.product.id),
            product=model.product.label)
        )

    def format_basket(self, context, model, name):
        return Markup('<a href="{url}" title="Link to Basket">{basket}</a>'.format(
