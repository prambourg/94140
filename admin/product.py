...
class ProductAdminView(AethModelView):
    ...
    column_sortable_list = ['n_total', ]

    column_details_list = ('label',
                           'description',
                           'category',
                           'glucide',
                           'protide',
    ...
    def details_view(self):
        data = [{"x": int(str(int(purchase.basket.date.timestamp())) + '000'), "y": purchase.normalized_price}
                for purchase
                in Purchase.query.filter(Purchase.product_id == request.args.get('id', ''))]
